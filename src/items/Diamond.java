...
public class Diamond {
    ...
    private BufferedImage diamondRaster;
    ...
    public Diamond(int x, int y, int xVel, float initialYVel, int value, GamePanel gp) {
        this.x = x;
        this.y = y;
        this.xVel = xVel;
    ...
    public void update() {
        if (y < gp.getPreferredSize().getHeight()) {
            ...
            if (showValueAnimationTick < 1) {
                gp.getPlayer().getPlayerData().addDiamonds(value);
            }
    ...
    public void initImages() {
        try {
            diamondRaster = ImageIO.read(getClass().getResourceAsStream("/items/diamond_raster.png"));
            animationList = new BufferedImage[4];

            for (int i = 0; i < animationList.length; i++) {
                animationList[i] = diamondRaster.getSubimage(i * 24, 0, 24, 24);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
